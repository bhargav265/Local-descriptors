...
train = pd.read_csv("LBP_features_train_normal.csv",header = None)
...
test = pd.read_csv("LBP_features_test_normal.csv", header = None).values
...
for h in range(10):
	...
	rf = XGBClassifier(n_estimators=est[h])
	rf.fit(train, target)
