	...
	for i in range(10):
		...
		for x in range(0,len(img)):
			for y in range(0,len(img[0])):
				...
				one = top_left - bottom_right
				two = top_up - bottom_down 
				three = top_right - bottom_left
				four = right - left 
				...
				values = thresholded([one, two, three, four])
				weights = [1, 2, 4, 8]
				res = 0
...
#read_img_center()
read_img()
#np.savetxt("CSLBPfeatures_train.csv",LGP_features, delimiter = ',')
#read_img_test()
#np.savetxt("CSLBPfeatures_test.csv",LGP_features_test, delimiter = ',')
